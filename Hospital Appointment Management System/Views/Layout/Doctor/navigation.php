...
<nav class="doctor-nav">
    ...
    <div class="nav-tabs">
        ...
        <a href="mainpage.php" class="nav-tab <?= $current_page == 'mainpage.php' ? 'active' : '' ?>">Dashboard</a>
        <a href="schedule.php" class="nav-tab <?= $current_page == 'schedule.php' ? 'active' : '' ?>">Schedule</a>
        <a href="appointments.php" class="nav-tab <?= $current_page == 'appointments.php' ? 'active' : '' ?>">Appointments</a>
        <a href="patients.php" class="nav-tab <?= $current_page == 'patients.php' ? 'active' : '' ?>">Patients</a>
        <a href="medicalRecords.php" class="nav-tab <?= ($current_page == 'medicalRecords.php' || $current_page == 'Patient_medicalRecords.php') ? 'active' : '' ?>">Medical Records</a>
    </div>
    ...
    <div class="nav-user">
        <div class="user-profile-container" style="position: relative;">
            <div class="user-profile" onclick="toggleProfileDropdown(event)" style="cursor:pointer; display: flex; align-items: center; gap: 8px; padding: 6px 12px; border-radius: 8px; transition: background 0.2s;" onmouseover="this.style.backgroundColor='var(--slate-100)'" onmouseout="this.style.backgroundColor='transparent'">
                <div class="user-avatar" style="background-color: var(--teal); color: var(--white); margin-right: 0;">
                    <?= isset($_SESSION['user']['username']) ? e(strtoupper(substr($_SESSION['user']['username'], 0, 1))) : 'D' ?>
                </div>
                <span class="user-name">
                    <?= isset($_SESSION['user']['username']) ? e($_SESSION['user']['username']) : 'Dr. Sarah Johnson' ?>
                </span>
            ...
            <div class="profile-dropdown-menu" id="profileDropdownMenu" style="display: none; position: absolute; top: 100%; right: 0; margin-top: 8px; background: white; border: 1px solid var(--slate-200); border-radius: 8px; width: 160px; box-shadow: 0 10px 15px -3px rgba(0,0,0,0.1), 0 4px 6px -2px rgba(0,0,0,0.05); z-index: 1000; overflow: hidden;">
                <a href="doctorProfile.php" style="display: flex; align-items: center; gap: 8px; padding: 10px 14px; font-size: 13px; color: var(--slate-700); text-decoration: none; transition: background 0.15s;" onmouseover="this.style.backgroundColor='var(--slate-50)'" onmouseout="this.style.backgroundColor='transparent'">
